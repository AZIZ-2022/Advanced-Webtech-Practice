...
export class AuthService {
  ...
  async signup(dto: createuser) {
    ...
    const payload = { email: dto.email, timestamp: Date.now() };
    const jwtSignupToken = this.jwtService.sign(payload);
  
    const newUser = this.userRepo.create({
      ...dto,
      password: hashedPassword,
      signupToken: jwtSignupToken,
    });
    await this.userRepo.save(newUser);
  
    return { message: 'Signup successful!', signupToken: jwtSignupToken };
  }
